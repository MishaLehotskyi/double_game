...
export default function StandardBank() {
  ...
  useEffect(() => {
    api.get('game/latest/STANDARD').then((res) => {
  ...
  useEffect(() => {
    api.get('game/finished?type=STANDARD').then((res) => {
      ...
      <div
        ...
        <div
          ...
          <Tooltip
            open={open}
            onClick={handleClick}
            title={<p className={"text-[18px]"} >Для участия в лотереи Standard Bank переведите 500 токенов DBE на адрес <span className={"text-green-600"} >0x0A59e974890265660BC9f3c2182e5cAA9c036723</span><Copy text={"0x0A59e974890265660BC9f3c2182e5cAA9c036723"} white={true} /><br/> Сеть BNB Smart Chain (BEP 20). После этого вы автоматически появитесь в окне участников с присвоенным номером билета</p>}
        ...
        <div className={"flex md:justify-evenly justify-between items-center w-full px-[15px]"}>
          <div className={"flex flex-col justify-center items-center gap-[10px]"}>
            ...
            <p className="text-[60px] font-bold border border-green-600 rounded-2xl md:px-[15px] px-[5px] min-w-[80px] text-center">{tickets.length}</p>
          </div>
          ...
          <div className={"flex flex-col justify-center items-center gap-[10px]"}>
            ...
            {user !== null && tickets.findIndex(t => t.metamaskId.toLowerCase() === user.metamaskId.toLowerCase()) !== -1
              ? <p
                className="text-[60px] font-bold border border-green-600 rounded-2xl md:px-[15px] px-[5px] min-w-[80px] text-center">{tickets.findIndex(t => t.metamaskId.toLowerCase() === user.metamaskId.toLowerCase()) + 1}</p>
              : <p
        ...
        {currentStep >= 4 && (<p className={"text-xl md:text-3xl"} >Следующая игра через: <Timer color="green-600" miliseconds={600000} showHours={false} /></p>)}
        <div className={"flex flex-col justify-center items-center md:gap-[30px] gap-[15px]"}>
          ...
          <div
            className={"border border-green-600 md:w-[500px] w-[350px] md:h-[200px] h-[120px] p-[10px] overflow-x-hidden overflow-y-auto scrollbar-custom"}>
            {tickets.length > 0 && tickets.map((ticket, index) => (
              <div key={ticket.id}
                className={`flex justify-center text-[15px] md:text-[20px] gap-[15px] ${user?.metamaskId && ticket.metamaskId.toLowerCase() === user.metamaskId.toLowerCase() ? 'bg-green-600 rounded-2xl px-[5px]' : ''}`}>
          ...
          <div
            className={"flex flex-col justify-center items-center gap-[30px] md:border-2 md:border-green-600 md:shadow-[0_0_20px_5px_rgba(0,255,0,0.5)] p-[20px]"}>
            <h1 className={"text-center text-green-600 "}>ВЫПЛАТЫ ПОБЕДИТЕЛЯМ</h1>
            {results.length > 0 ?
              results.map((result) => (
                <>
                  ...
                  <div
                    ...
                    <p
                      className={"md:w-[300px] w-[250px] md:text-xl text-base border-2 border-green-600 shadow-[0_0_20px_5px_rgba(0,255,0,0.5)] rounded-2xl p-[10px] transition-transform duration-300 hover:scale-110"}>
                      1) Место: <a
                        href={`https://bscscan.com/tx/${result.winners[0].payHash}`}
                        target={"_blank"} className={"text-green-600 cursor-pointer"}>Хэш транзакции</a> Сумма: 3000 DBE
                    </p>
                    <p
                      className={"md:w-[300px] w-[250px] md:text-xl text-base border-2 border-green-600 shadow-[0_0_20px_5px_rgba(0,255,0,0.5)] rounded-2xl p-[10px] transition-transform duration-300 hover:scale-110"}>
                      2) Место: <a
                        href={`https://bscscan.com/tx/${result.winners[1].payHash}`}
                        target={"_blank"} className={"text-green-600 cursor-pointer"}>Хэш транзакции</a> Сумма: 750 DBE
                    </p>
                    <p
                      className={"md:w-[300px] w-[250px] md:text-xl text-base border-2 border-green-600 shadow-[0_0_20px_5px_rgba(0,255,0,0.5)] rounded-2xl p-[10px] transition-transform duration-300 hover:scale-110"}>
                      3) Место: <a
                        href={`https://bscscan.com/tx/${result.winners[2].payHash}`}
                        target={"_blank"} className={"text-green-600 cursor-pointer"}>Хэш транзакции</a> Сумма: 500 DBE
                    </p>
                  </div>
                </>
              ))
